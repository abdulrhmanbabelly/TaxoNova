...
export class ClassifyImagesState extends UIState {
  ...
  private updateDisplay() {
    ...
    if (this.manager.imagesRemaining() === true) {
      ...
      this.imageProgress.html(
        `Image&nbsp;${this.manager.getCurrentImageIndex() + 1}&nbsp;of&nbsp;${
          this.manager.getInputFiles().length
        }:`
      );

      //Determine the maximum height that the image cannot exceed
      let maxHeight =
        <number>$("body").innerHeight() - <number>this.undoOuter.innerHeight();
    ...
    if (lastAction !== null) {
      ...
      if (lastAction.destFile.length > 0) {
        this.undoText.text(
          `Classified the image "${path.basename(lastAction.sourceFile)}" as "${
            lastAction.label
          }".`
        );
      } else {
        this.undoText.text(
          `Ignored the image "${path.basename(lastAction.sourceFile)}".`
        );
      }
  ...
  private populateRoot() {
    ...
    let leftColumn = $(document.createElement("div")).attr(
      "id",
      "left-column-wrapper"
    );
    ...
    let imageWrapper = $(document.createElement("div")).attr(
      "id",
      "image-wrapper"
    );
    this.imageDisplay = $(document.createElement("img")).attr(
      "id",
      "current-image"
    );
    ...
    //Create our "undo" bar
    this.undoOuter = $(document.createElement("div")).attr(
      "id",
      "undo-wrapper"
    );
    ...
    let rightColumn = $(document.createElement("div")).attr(
      "id",
      "right-column-wrapper"
    );
    this.buttonWrapper = $(document.createElement("div")).attr(
      "id",
      "button-wrapper"
    );
    this.imageProgress = $(document.createElement("p")).attr(
      "id",
      "image-progress"
    );
    ...
    let ignoreButton = $(document.createElement("button")).html(
    ...
    // Create a button to ignore the current image
    let updateLablesButton = $(document.createElement("button")).html(
      "Update labels"
    );
    updateLablesButton.insertBefore(this.buttonWrapper);
    updateLablesButton.click(() => {
      this.updateLabels();
    });

    //Create our list of classification buttons
    this.createListOfClassificationButtons();
    //Update our image display to show the initial image
    this.updateDisplay();
  }

  public createListOfClassificationButtons() {
    // empty the wrapper so no button is repeated
  ...
  public async updateLabels() {
    // get new labels
    const labelSet = await ClassificationLabelManager.loadSupportedLabels();
    const labelArray = Array.from(labelSet)[0][1]; // Direct conversion from Set to Array
    this.manager.updateLabels(labelArray);
    // update ui
    this.createListOfClassificationButtons();
  }
}
