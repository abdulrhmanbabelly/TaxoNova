...
export class SelectParametersState extends UIState {
  ...
  private createDirChooser(fieldID: string, fieldLabel: string) {
    ...
    let field = $(document.createElement("input"))
      .attr("type", "hidden")
      .attr("id", fieldID);
    let display = $(document.createElement("span")).text(
    ...
    button.click(() => {
      this.dialogs
        .showOpenDialog("Choose directory", [], true)
        .then((paths: string[]) => {
  ...
  private populateRoot() {
    ...
    //Create our heading
    let heading = $(document.createElement("h1")).text(
      "Select classification labels and data directories"
    );
    this.root.append(heading);

    //Create our labels dropdown
    let dropdownWrapper = $(document.createElement("p"));
    let dropdownLabel = $(document.createElement("strong")).text(
      "Classification labels:"
    );
    this.labelsDropdown = $(document.createElement("select")).attr(
      "id",
      "selected-labels"
    );
    ...
    let showDirButton = $(document.createElement("button")).text(
      "Show label files"
    );
    ...
    let inputDirChooser = this.createDirChooser(
      this.inputDirID,
      "Input directory: "
    );
    ...
    let outputDirChooser = this.createDirChooser(
      this.outputDirID,
      "Output directory: "
    );
    ...
    this.startButton = $(document.createElement("button")).text(
