...
export class ClassificationManager {
  ...
  protected async scanInputFiles() {
    try {
      ...
      for (let extension of ["gif", "jpg", "jpeg", "png"]) {
        this.inputFiles = this.inputFiles.concat(
          await globFiles(path.join(this.inputDir, `*.${extension}`))
        );
      }
      ...
      if (this.inputFiles.length == 0) {
        throw new Error(
          "No image files could be found in the input directory."
        );
      }
    } catch (err) {
      //Propagate any errors
  ...
  public async ignoreCurrentImage() {
    ...
    this.classifications.push(
      new ClassificationChoice(this.getCurrentImagePath(), "", "")
    );
  }
  ...
  public async classifyCurrentImage(label: string) {
    try {
      ...
      let pathBase = Utility.removeExtension(
        path.join(this.subdirectoryForLabel(label), path.basename(sourceFile))
      );
      ...
      let classification = new ClassificationChoice(
        sourceFile,
        destFile,
        label
      );
  ...
  public async undoLastClassification() {
    try {
      ...
      if (this.classifications.length == 0) {
        throw new Error(
          "No existing classification choices available to undo."
        );
      }
